...
namespace MVC5_Full_template.Services.Sitemap
{
    using System;
    ...
    using MVC5_Full_template.Constants;
    ...
    public class SitemapService : SitemapGenerator, ISitemapService
    {
        ...
        public string GetSitemapXml(int? index = null)
        {
            ...
            var sitemapDocuments = this._cacheService.GetOrAdd(
        ...
        protected virtual IReadOnlyCollection<SitemapNode> GetSitemapNodes()
        {
            ...
            nodes.Add(
                new SitemapNode(this._urlHelper.AbsoluteRouteUrl(HomeControllerRoute.GetIndex))
                {
                    Priority = 1
                });
            nodes.Add(
               new SitemapNode(this._urlHelper.AbsoluteRouteUrl(HomeControllerRoute.GetAbout))
               {
                   Priority = 0.9
               });
            nodes.Add(
               new SitemapNode(this._urlHelper.AbsoluteRouteUrl(HomeControllerRoute.GetContact))
               {
                   Priority = 0.9
               });
        ...
        protected override string GetSitemapUrl(int index) => this._urlHelper.AbsoluteRouteUrl(HomeControllerRoute.GetSitemapXml).TrimEnd('/') + "?index=" + index;

        protected override void LogWarning(Exception exception)
        {
            this._loggingService.Log(exception);
        }
